...
if __name__ == "__main__":
	...
	# Load test image!
	i=0
	numpy_=np.zeros((1,1537))
	for filename in os.listdir('/home/kog1cob/Dheeraj/iCubWorld28_128x128/'):
		if(filename!='.DS_Store'):
			for files in os.listdir('/home/kog1cob/Dheeraj/iCubWorld28_128x128/'+filename):
				if(filename!='.DS_Store'):
					img = get_processed_image('/home/kog1cob/Dheeraj/iCubWorld28_128x128/'+filename+'/'+files)
					preds = model.predict(img)
					abc=np.array([[i]])
					print(abc)
					preds=np.concatenate((abc,preds),axis=1)
					print(numpy_.shape,preds.shape)
					numpy_=np.concatenate((numpy_,preds),axis=0)
		i+=1
		print(i)
	np.save('/home/kog1cob/Dheeraj/FALKON-master/trainnumpy.npy',numpy_)
